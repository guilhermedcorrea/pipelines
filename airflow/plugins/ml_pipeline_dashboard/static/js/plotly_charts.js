(function (window) {
  ...
  function exibirMensagem(container, mensagem) {
    if (!container) return;
    container.innerHTML = `<div class="ml-estado-vazio">${utils.escaparHtml(mensagem)}</div>`;
  }

  function layoutBase(titulo) {
    return {
      title: { text: titulo || "", font: { color: "#e8eeff", size: 16 } },
      paper_bgcolor: "rgba(0,0,0,0)",
      plot_bgcolor: "rgba(0,0,0,0)",
      font: { color: "#e8eeff", family: "Inter, Segoe UI, Arial, sans-serif" },
      margin: { t: 46, r: 20, b: 50, l: 50 },
      xaxis: {
        gridcolor: "rgba(255,255,255,.08)",
        zerolinecolor: "rgba(255,255,255,.10)",
        tickfont: { color: "#a8b3d1" },
        titlefont: { color: "#a8b3d1" },
      },
      yaxis: {
        gridcolor: "rgba(255,255,255,.08)",
        zerolinecolor: "rgba(255,255,255,.10)",
      ...
      legend: { font: { color: "#e8eeff" } },
      hoverlabel: { bgcolor: "#11182d", bordercolor: "#26345c", font: { color: "#e8eeff" } },
    };
  }
  ...
  function renderizarBarrasMetricas(container, itens, titulo = "Métricas principais por task") {
    ...
    const linhas = utils.arraySeguro(itens).filter((item) => Number.isFinite(Number(item?.valor)));
    if (!linhas.length) {
    ...
    const hover = linhas.map((item) => `${item.task} • ${item.metric_label}: ${utils.formatarNumero(item.valor, 4)}`);
    ...
    window.Plotly.newPlot(
      ...
      [{
        ...
        marker: {
          ...
          line: { color: "rgba(34,211,238,.45)", width: 1 },
        },
      }],
      {
        ...layoutBase(titulo),
        yaxis: { ...layoutBase(titulo).yaxis, title: "Valor" },
        xaxis: { ...layoutBase(titulo).xaxis, title: "Task / Métrica" },
      },
      configuracaoBase(),
    );
  }
  ...
  function renderizarSerieFolds(container, itens, titulo = "Evolução das métricas por fold") {
    ...
    linhas.forEach((item) => {
      ...
      if (!agrupado.has(chave)) {
        agrupado.set(chave, { nome: `${item.task} • ${item.metric_label}`, x: [], y: [] });
      }
    ...
    window.Plotly.newPlot(
      ...
      {
        ...layoutBase(titulo),
        xaxis: { ...layoutBase(titulo).xaxis, title: "Fold" },
        yaxis: { ...layoutBase(titulo).yaxis, title: "Valor" },
      },
      configuracaoBase(),
    );
