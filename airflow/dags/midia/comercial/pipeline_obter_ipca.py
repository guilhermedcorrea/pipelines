"""
DAG: pipeline_obter_ipca

Carga do IPCA mensal do Banco Central do Brasil para a tabela
[DataMining].[Silver].[FatoIPCA] no SQL Server.
...
Pipeline responsável por obter a série mensal do **IPCA** no SGS/BCData do
Banco Central do Brasil e gravar os dados tratados na tabela
`[DataMining].[Silver].[FatoIPCA]`.
...
Exemplo: `0.26` significa `0,26%`, não `26%` e não `0,26` em decimal.
"""
...
def normalizar_linha_stage_ipca(linha: Tuple) -> Tuple:
...
def diagnosticar_linhas_ipca(linhas: List[Tuple]) -> None:
    ...
    print("=" * 100)
    print("DIAGNOSTICO DAS PRIMEIRAS LINHAS GERADAS PARA O IPCA")
    print("=" * 100)
    ...
    for idx, linha in enumerate(linhas[:5], start=1):
        ...
        print(f"  IPCAVariacaoMensalPct: {linha[6]}")
        print(f"  IPCAVariacaoMensalDecimal: {linha[7]}")
        print(f"  IPCAFatorMensal: {linha[8]}")
        print(f"  IPCAAcumulado12MPct: {linha[11]}")
        print("-" * 100)
...
def imprimir_ultima_data_bcb(registros: List[Dict[str, Any]], hoje: date) -> None:
    ...
    if ultima_data_bcb < primeiro_dia_mes(hoje):
        print(
            "AVISO: A serie do IPCA e mensal e pode ter defasagem de publicacao. "
...
def validar_tabela_fato_ipca(cursor) -> None:
    """
    ...
    cursor.execute(
        """
        IF OBJECT_ID('[Silver].[FatoIPCA]', 'U') IS NULL
        BEGIN
            RAISERROR('A tabela [DataMining].[Silver].[FatoIPCA] nao existe no database conectado. Crie a tabela antes de executar a carga ou verifique a Connection do Airflow.', 16, 1);
        END
        """
    )
...
def buscar_ipca_bcb(data_inicial: date, data_final: date) -> Tuple[List[Dict[str, Any]], str, int, str]:
    codigo_serie = 433
        ...
        registros.append(
            {
                ...
                "ipca_variacao_mensal_pct": valor_percentual,
            }
        )
...
def calcular_campos_ipca(
    ...
    observacao = (
        "IPCAVariacaoMensalPct vem da API do Banco Central e ja esta em percentual mensal. "
        "IPCAVariacaoMensalDecimal = percentual / 100. "
        "IPCAFatorMensal = 1 + decimal. "
    ...
    for i, registro in enumerate(registros):
        ...
        variacao_mensal_pct = registro["ipca_variacao_mensal_pct"]
        ...
        if i >= 2:
            percentuais_3m = [registros[j]["ipca_variacao_mensal_pct"] for j in range(i - 2, i + 1)]
            acumulado_3m_pct = calcular_acumulado_pct(percentuais_3m)

        acumulado_6m_pct = None
        if i >= 5:
            percentuais_6m = [registros[j]["ipca_variacao_mensal_pct"] for j in range(i - 5, i + 1)]
            acumulado_6m_pct = calcular_acumulado_pct(percentuais_6m)

        acumulado_12m_pct = None
        if i >= 11:
            percentuais_12m = [registros[j]["ipca_variacao_mensal_pct"] for j in range(i - 11, i + 1)]
            acumulado_12m_pct = calcular_acumulado_pct(percentuais_12m)

        percentuais_ano = [
            registros[j]["ipca_variacao_mensal_pct"]
        ...
        if i >= 2:
            media_movel_3m = calcular_media(
                [
                    registros[i - 2]["ipca_variacao_mensal_pct"],
                    registros[i - 1]["ipca_variacao_mensal_pct"],
                    registros[i]["ipca_variacao_mensal_pct"],
                ]
            )
        ...
        if i >= 5:
            media_movel_6m = calcular_media(
                [registros[j]["ipca_variacao_mensal_pct"] for j in range(i - 5, i + 1)]
            )

        media_movel_12m = None
        if i >= 11:
            media_movel_12m = calcular_media(
                [registros[j]["ipca_variacao_mensal_pct"] for j in range(i - 11, i + 1)]
            )
...
def upsert_fato_ipca(cursor, linhas: List[Tuple]) -> int:
    if not linhas:
        return 0
    ...
    cursor.execute(
        """
        IF OBJECT_ID('tempdb..#StageFatoIPCA') IS NOT NULL
        BEGIN
            DROP TABLE #StageFatoIPCA;
        END
        """
    )
    ...
    cursor.execute(
        ...
        (
            ...
            [IPCAVariacaoMensalPct] DECIMAL(18,6) NOT NULL,
            [IPCAVariacaoMensalDecimal] DECIMAL(18,10) NULL,
            [IPCAFatorMensal] DECIMAL(18,10) NULL,
            [IPCAAcumulado3MPct] DECIMAL(18,6) NULL,
            [IPCAAcumulado6MPct] DECIMAL(18,6) NULL,
            [IPCAAcumulado12MPct] DECIMAL(18,6) NULL,
            [IPCAAcumuladoAnoPct] DECIMAL(18,6) NULL,
            [IPCAMediaMovel3M] DECIMAL(18,6) NULL,
            [IPCAMediaMovel6M] DECIMAL(18,6) NULL,
            [IPCAMediaMovel12M] DECIMAL(18,6) NULL,
    ...
    insert_stage = """
        ...
        (
            ...
            Mes,
            MesAno,
            IPCAVariacaoMensalPct,
            IPCAVariacaoMensalDecimal,
            IPCAFatorMensal,
            IPCAAcumulado3MPct,
            IPCAAcumulado6MPct,
            IPCAAcumulado12MPct,
    ...
    linhas_normalizadas = [normalizar_linha_stage_ipca(linha) for linha in linhas]
    cursor.executemany(insert_stage, linhas_normalizadas)

    cursor.execute(
        """
        MERGE [Silver].[FatoIPCA] AS Destino
        USING #StageFatoIPCA AS Origem
        ...
        WHEN MATCHED THEN
            UPDATE SET
                ...
                Destino.IPCAVariacaoMensalPct = Origem.IPCAVariacaoMensalPct,
                Destino.IPCAVariacaoMensalDecimal = Origem.IPCAVariacaoMensalDecimal,
                Destino.IPCAFatorMensal = Origem.IPCAFatorMensal,
                Destino.IPCAAcumulado3MPct = Origem.IPCAAcumulado3MPct,
                Destino.IPCAAcumulado6MPct = Origem.IPCAAcumulado6MPct,
                Destino.IPCAAcumulado12MPct = Origem.IPCAAcumulado12MPct,
                Destino.IPCAAcumuladoAnoPct = Origem.IPCAAcumuladoAnoPct,
                Destino.IPCAMediaMovel3M = Origem.IPCAMediaMovel3M,
                Destino.IPCAMediaMovel6M = Origem.IPCAMediaMovel6M,
                Destino.IPCAMediaMovel12M = Origem.IPCAMediaMovel12M,
        ...
        WHEN NOT MATCHED THEN
            ...
            (
                ...
                Origem.Mes,
                Origem.MesAno,
                Origem.IPCAVariacaoMensalPct,
                Origem.IPCAVariacaoMensalDecimal,
                Origem.IPCAFatorMensal,
                Origem.IPCAAcumulado3MPct,
                Origem.IPCAAcumulado6MPct,
                Origem.IPCAAcumulado12MPct,
                Origem.IPCAAcumuladoAnoPct,
                Origem.IPCAMediaMovel3M,
                Origem.IPCAMediaMovel6M,
                Origem.IPCAMediaMovel12M,
    ...
    cursor.execute("DROP TABLE #StageFatoIPCA;")

    return len(linhas)


def executar_carga_ipca(conn_id_sql_server: str = CONN_ID_SQL_SERVER) -> Dict[str, Any]:
    """Executa a carga do IPCA usando o HookSqlServer do Airflow."""
    ...
    try:
        ...
        validar_tabela_fato_ipca(cursor)
        ...
        registros_bcb, url_consulta, codigo_serie, nome_serie = buscar_ipca_bcb(
        ...
        diagnosticar_linhas_ipca(linhas)
        qtd_linhas_upsert = upsert_fato_ipca(cursor, linhas)
        ...
        resumo = {
            ...
            "observacao": (
                "IPCAVariacaoMensalPct ja vem da API como percentual mensal. "
                "Exemplo: 0.26 significa 0,26%, nao 26% e nao 0,26 em decimal. "
                "O decimal gravado e IPCAVariacaoMensalDecimal = percentual / 100. "
...
@dag(
    dag_id=DAG_ID,
    description="Obtém o IPCA mensal do Banco Central e realiza upsert em [DataMining].[Silver].[FatoIPCA] no SQL Server.",
    schedule="0 9,19 * * *",
    start_date=pendulum.datetime(2026, 6, 11, tz=TZ_SAO_PAULO),
    catchup=False,
    max_active_runs=1,
    tags=["bcb", "sgs", "ipca", "sql-server", "silver", "macroeconomia"],
    default_args={
        "owner": "midia",
        "retries": 2,
...
def pipeline_obter_ipca():
    ...
    @task(task_id="obter_e_gravar_ipca")
    def obter_e_gravar_ipca() -> Dict[str, Any]:
        return executar_carga_ipca(conn_id_sql_server=CONN_ID_SQL_SERVER)

    testar_conexao_sql_server() >> obter_e_gravar_ipca()


pipeline_obter_ipca_dag = pipeline_obter_ipca()
