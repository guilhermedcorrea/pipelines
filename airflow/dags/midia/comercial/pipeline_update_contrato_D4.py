...
TABELA_CONTROLE_CONTRATOS = "[Integracao].[Silver].[FatoControleContratosMidia]"
TABELA_CONTROLE_CONTRATOS_ITENS = "[Integracao].[Silver].[FatoControleContratosItensMidia]"
TABELA_STATUS_CONTRATOS = "[Integracao].[Silver].[DimStatusContratos]"
TABELA_SOLICITACAO_CONTRATO = "[Integracao].[Silver].[FatoSolicitacaoContratoMidia]"
TABELA_CONTRATO_CARD = "[Integracao].[Silver].[FatoContratoCardMidia]"
...
TABELA_ARQUIVOS_CONTRATOS = "[Integracao].[Silver].[FatoArquivosContratosMidia]"
...
def pasta_pdf_local_contrato_d4() -> Path:
    ...
    pasta = str(
        ...
        or "/opt/airflow/FlaskApp/Contratos/Midia/Anexos/Contrato"
    ).strip()
    return Path(pasta)
...
def montar_nome_pdf_local_contrato_d4(contrato: dict[str, Any], nome_documento_d4: str | None = None) -> str | None:
    id_controle = converter_int(contrato.get("IDFatoControleContratosMidia"))
    ...
    linha = conexao.execute(
        text(f"""
            SELECT TOP (1)
             ...
             WHERE IDFatoControleContratosMidia = :id_controle
               AND (
...
def registrar_pdf_tabela_arquivos(conexao, *, contrato: dict[str, Any], nome_arquivo: str, tamanho_arquivo: int) -> dict[str, Any]:
    ...
    id_d4 = converter_int(contrato.get("IDFatoContratoD4"))
    if not id_controle:
        raise RuntimeError("Sem IDFatoControleContratosMidia para gravar FatoArquivosContratosMidia.")
    if not id_d4:
        raise RuntimeError("Sem IDFatoContratoD4 para gravar FatoArquivosContratosMidia.")
        ...
        conexao.execute(
            text(f"""
                UPDATE {TABELA_ARQUIVOS_CONTRATOS}
                   SET IDFatoControleContratosMidia = :id_controle,
...
def garantir_pdf_local_contrato_d4(
    ...
    *,
    ...
    """Garante o registro em FatoArquivosContratosMidia e, quando possível, o PDF físico.

    Regra aplicada:
    - contrato existe em FatoControleContratosMidia;
    - card existe/vem vinculado;
    - documento existe na FatoContratoD4 com UUIDDocumentoD4;
    - se não existir linha na [Integracao].[Silver].[FatoArquivosContratosMidia], insere a linha;
    ...
    # 3) Se NÃO existe na tabela correta, insere primeiro na FatoArquivosContratosMidia.
    ...
    linha = conexao.execute(
        text(f"""
            SELECT TOP (1)
                ...
                d.IDDimCofreD4,
                d.IDFatoControleContratosMidia,
            ...
            OUTER APPLY (
                ...
                WHERE cc.IDFatoControleContratosMidia = d.IDFatoControleContratosMidia
                  AND cc.IDFatoKanbanCard IS NOT NULL
                ORDER BY
                    cc.DataAtualizacao DESC,
                    cc.IDFatoContratoCardMidia DESC
            ) card
            WHERE ISNULL(d.BitAtivo, 1) = 1
              ...
              AND (
                 ...
                 OR (:id_fato_controle_contrato IS NOT NULL AND d.IDFatoControleContratosMidia = :id_fato_controle_contrato)
              )
            ORDER BY
...
def garantir_pdf_local_contrato_d4_gravado(
    ...
    *,
    ...
    contrato_pdf = buscar_contrato_d4_para_pdf_por_id(
        ...
        id_fato_controle_contrato=contrato_gravado.get("IDFatoControleContratosMidia"),
    )
    if not contrato_pdf:
...
def garantir_pdfs_locais_contratos_d4(
    ...
    *,
    ...
    1. parte do contrato da Euromídia ativo em FatoControleContratosMidia;
    2. exige vínculo com card em FatoContratoCardMidia;
    3. exige documento gravado em FatoContratoD4 com UUIDDocumentoD4;
    4. verifica se falta linha/informação em FatoArquivosContratosMidia;
    ...
    contratos = hook_sql.executar_select(
        ...
        ;WITH ContratosComCard AS (
            SELECT
                c.IDFatoControleContratosMidia,
                ...
                card.IDFatoContratoCardMidia,
                card.IDFatoControleContratosItensMidia,
                card.IDFatoKanbanCard
            FROM {TABELA_CONTROLE_CONTRATOS} c
            OUTER APPLY (
                SELECT TOP (1)
                    cc.IDFatoContratoCardMidia,
                    cc.IDFatoControleContratosItensMidia,
                    cc.IDFatoKanbanCard
                FROM {TABELA_CONTRATO_CARD} cc WITH (READPAST)
                WHERE cc.IDFatoControleContratosMidia = c.IDFatoControleContratosMidia
                  AND cc.IDFatoKanbanCard IS NOT NULL
                ORDER BY
                    cc.DataAtualizacao DESC,
                    cc.IDFatoContratoCardMidia DESC
        ...
        ContratosD4ComCard AS (
            SELECT
                ...
                d.IDEmpresa AS IDEmpresa,
                d.IDDimCofreD4,
                cc.IDFatoControleContratosMidia,
                cc.IDFatoContratoCardMidia,
                cc.IDFatoControleContratosItensMidia,
            ...
            INNER JOIN {TABELA_CONTRATO_D4} d WITH (READPAST)
                ON d.IDFatoControleContratosMidia = cc.IDFatoControleContratosMidia
               AND ISNULL(d.BitAtivo, 1) = 1
               AND NULLIF(LTRIM(RTRIM(d.UUIDDocumentoD4)), '') IS NOT NULL
            OUTER APPLY (
                SELECT TOP (1)
                    a.IDFatoArquivosContratos,
                    a.IDFatoControleContratosMidia,
                    a.IDFatoKanbanCard,
                    a.IDFatoContratoD4,
                    a.NomeArquivo,
                    a.UrlAnexo,
                ...
                WHERE (
                     ...
                     OR (
                        ...
                        AND a.IDFatoControleContratosMidia = cc.IDFatoControleContratosMidia
                        )
                      )
                ORDER BY
    ...
    for contrato in contratos:
        try:
            ...
            logging.exception(
                ...
                contrato.get("IDFatoControleContratosMidia"),
                contrato.get("IDFatoKanbanCard"),
                contrato.get("UUIDDocumentoD4"),
            )
            erros.append({
                "IDFatoContratoD4": contrato.get("IDFatoContratoD4"),
                "IDFatoControleContratosMidia": contrato.get("IDFatoControleContratosMidia"),
...
TAGS_DAG = ["Midia", "Contratos", "D4Sign", "SQLServer", "API"]
...
8. Propaga o status para `[Integracao].[Silver].[FatoControleContratosMidia]`.
9. Sincroniza o texto de status em `[Integracao].[Silver].[FatoControleContratosItensMidia]`.
...
def resolver_status_final_midia(
    status_atual: Any,
    status_novo: Any,
...
def buscar_status_atual_controle_midia(conexao, id_fato_controle_contrato: int) -> int | None:
    """Leio o status atual da tabela principal da esteira do contrato."""
    sql = text(f"""
        SELECT TOP (1)
            IDDimStatusContratos
        FROM {TABELA_CONTROLE_CONTRATOS} WITH (UPDLOCK, ROWLOCK)
        WHERE IDFatoControleContratosMidia = :IDFatoControleContratosMidia
          AND ISNULL(BitAtivo, 1) = 1;
    """)
    linha = conexao.execute(
        sql,
        {"IDFatoControleContratosMidia": id_fato_controle_contrato},
    ).mappings().first()
...
def inserir_historico_d4(conexao, parametros: dict[str, Any]) -> dict[str, Any]:
    ...
    id_controle = converter_int(parametros.get("IDFatoControleContratosMidia"))
    ...
    sql = text(f"""
        ;WITH UltimoHistorico AS (
            ...
            WHERE IDEmpresaProprietaria = :IDEmpresaProprietaria
              AND IDFatoControleContratosMidia = :IDFatoControleContratosMidia
    ...
    linha = conexao.execute(
        sql,
        {
            "IDEmpresaProprietaria": ID_EMPRESA_PROPRIETARIA_MIDIA,
            "IDFatoControleContratosMidia": id_controle,
...
def propagar_status_contrato_midia(
    conexao,
    *,
    ...
    status_atual = buscar_status_atual_controle_midia(conexao, id_controle)
    status_final = resolver_status_final_midia(status_atual, id_status_novo, id_d4)
    ...
    sql_itens = text(f"""
        ...
        WHERE itens.IDFatoControleContratoMidia = :IDFatoControleContratosMidia
    ...
    sql_solicitacao = text(f"""
        ...
        WHERE ISNULL(solicitacao.BitAtivo, 1) = 1
          AND (
                solicitacao.IDFatoControleContratosMidia = :IDFatoControleContratosMidia
                OR (
...
def sincronizar_estado_local_d4_com_esteira(hook_sql: HookSqlServer) -> dict[str, Any]:
    ...
    contratos = hook_sql.executar_select(
        ...
        WHERE ISNULL(BitAtivo, 1) = 1
          AND IDFatoControleContratosMidia IS NOT NULL
    ...
    with engine.begin() as conexao:
        for contrato in contratos:
            ...
            resultados.append(
                propagar_status_contrato_midia(
                    conexao,
                    id_fato_controle_contrato=contrato.get("IDFatoControleContratosMidia"),
    ...
    def verificar_e_atualizar_status_contratos() -> dict[str, Any]:
        ...
        for contrato in contratos:
            ...
            try:
                ...
                metadados_api["IDFatoControleContratosMidia"] = contrato.get("IDFatoControleContratosMidia")
                ...
                for parametros in atualizacoes:
                    ...
                    try:
                        ...
                        logging.exception(
                            ...
                            parametros.get("IDFatoControleContratosMidia"),
                        )
                        erros_pdf_contratos_gravados.append({
                            "IDFatoContratoD4": parametros.get("IDFatoContratoD4"),
                            "IDFatoControleContratosMidia": parametros.get("IDFatoControleContratosMidia"),
        ...
        if propagacoes_para_controle:
            ...
            with engine.begin() as conexao:
                for parametros in propagacoes_para_controle:
                    propagacao_midia = propagar_status_contrato_midia(
                        conexao,
                        id_fato_controle_contrato=parametros.get("IDFatoControleContratosMidia"),
                    ...
                    resultados_propagacao.append(propagacao_midia)
        ...
        resumo = {
            ...
            "propagacoes_midia": len(resultados_propagacao),
            "resultados_propagacao_midia": resultados_propagacao[:50],
