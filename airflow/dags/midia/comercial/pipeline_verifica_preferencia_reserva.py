...
`Silver.FatoControleContratosMidia.IDDimStatusContratos`
...
2. Marca `Silver.FatoControleContratosItensMidia.BitPreferencia = 1` para itens elegíveis.
...
`IDFatoControleContratosItensMidia`. Por isso, a sincronização dos itens usa uma chave lógica.
...
o ideal técnico é adicionar `IDFatoControleContratosItensMidia` na tabela de destino.
"""
...
class ConfiguracaoPreferenciaReserva:
    ...
    tabela_contratos: str = "[Integracao].[Silver].[FatoControleContratosMidia]"
    tabela_contratos_itens: str = "[Integracao].[Silver].[FatoControleContratosItensMidia]"
...
def validar_estrutura_tabelas() -> dict[str, Any]:
    ...
    (
        ...
        (
            VALUES
                (N'FatoControleContratosMidia', N'IDFatoControleContratosMidia'),
                (N'FatoControleContratosMidia', N'DataAtualizacao'),
                (N'FatoControleContratosMidia', N'IDEmpresa'),
                (N'FatoControleContratosMidia', N'IDDimStatusContratos'),
                (N'FatoControleContratosMidia', N'BitAtivo'),
                (N'FatoControleContratosMidia', N'TotalLiquidoContratoAGBRVENDGERCOOR'),
                (N'FatoControleContratosMidia', N'MarcaExibida'),

                (N'FatoControleContratosItensMidia', N'IDFatoControleContratosItensMidia'),
                (N'FatoControleContratosItensMidia', N'IDFatoControleContratoMidia'),
                (N'FatoControleContratosItensMidia', N'DataInicioPrevisto'),
                (N'FatoControleContratosItensMidia', N'DataTerminoPrevisto'),
                (N'FatoControleContratosItensMidia', N'IDPainelMidia'),
                (N'FatoControleContratosItensMidia', N'IDDimFacesPaineis'),
                (N'FatoControleContratosItensMidia', N'IDVendedor'),
                (N'FatoControleContratosItensMidia', N'BitAtivo'),
                (N'FatoControleContratosItensMidia', N'BitPreferencia'),
                (N'FatoControleContratosItensMidia', N'BitOcupacaoFatiada'),
                (N'FatoControleContratosItensMidia', N'DataInicioTotalAgendamento'),
                (N'FatoControleContratosItensMidia', N'DataTerminoTotalAgendamento'),

                (N'Vendedores', N'IDVendedor'),
                (N'Vendedores', N'IDDimUsuarios'),

                (N'FatoPreferenciaReserva', N'IDFatoPreferenciaReserva'),
                (N'FatoPreferenciaReserva', N'IDFatoControleContratosMidia'),
                ...
                (N'FatoPreferenciaReservaItens', N'IDPainelMidia'),
    ...
    (
        ...
        WHERE
            (
                ...
                AND t.name IN
                (
                    N'FatoControleContratosMidia',
                    N'FatoControleContratosItensMidia',
...
def sincronizar_status_contratos_por_periodo(conexao, config: ConfiguracaoPreferenciaReserva) -> dict[str, int]:
    ...
    (
        SELECT
            item.IDFatoControleContratoMidia,
    ...
    SELECT
        contrato.IDFatoControleContratosMidia,
    ...
    INNER JOIN StatusContrato AS status_contrato
        ON status_contrato.IDFatoControleContratoMidia = contrato.IDFatoControleContratosMidia
    WHERE
    ...
    INNER JOIN #StatusContratosAtualizar AS status_atualizar
        ON status_atualizar.IDFatoControleContratosMidia = contrato.IDFatoControleContratosMidia;
    """
...
def criar_tabelas_temporarias_preferencia(conexao, config: ConfiguracaoPreferenciaReserva) -> dict[str, int]:
    ...
    INNER JOIN {config.tabela_contratos} AS contrato
        ON contrato.IDFatoControleContratosMidia = item.IDFatoControleContratoMidia
    ...
    ON #ItensPreferenciaReserva (IDFatoControleContratosItensMidia);
    ...
    (
        IDFatoControleContratosMidia INT NOT NULL PRIMARY KEY,
        IDEmpresa INT NULL,
        IDVendedor INT NULL,
        IDUsuario INT NULL,
    ...
    (
        SELECT
            ...
            ROW_NUMBER() OVER
            (
                PARTITION BY item.IDFatoControleContratoMidia
                ORDER BY
                    CASE WHEN item.IDUsuario IS NULL THEN 1 ELSE 0 END,
                    item.IDFatoControleContratosItensMidia DESC
            ) AS OrdemVendedor
        FROM #ItensPreferenciaReserva AS item
    )
    ...
    INNER JOIN contratos_elegiveis AS item_contrato
        ON item_contrato.IDFatoControleContratoMidia = contrato.IDFatoControleContratosMidia
    LEFT JOIN vendedor_contrato
        ON vendedor_contrato.IDFatoControleContratoMidia = contrato.IDFatoControleContratosMidia
...
def sincronizar_bit_preferencia_origem(conexao, config: ConfiguracaoPreferenciaReserva) -> dict[str, int]:
    ...
    INNER JOIN #ItensPreferenciaReserva AS preferencia
        ON preferencia.IDFatoControleContratosItensMidia = item.IDFatoControleContratosItensMidia
    ...
    WHERE
        ...
        (
            ...
            WHERE preferencia.IDFatoControleContratosItensMidia = item.IDFatoControleContratosItensMidia
        );
    """
...
def remover_preferencias_invalidas(conexao, config: ConfiguracaoPreferenciaReserva) -> dict[str, int]:
    ...
    (
        ...
        WHERE
            origem_item.IDFatoControleContratoMidia = destino.IDFatoControleContratosMidia
            ...
            AND ISNULL(origem_item.IDPainelMidia, -1) = ISNULL(destino_item.IDPainelMidia, -1)
    ...
    (
        ...
        WHERE origem.IDFatoControleContratosMidia = destino.IDFatoControleContratosMidia
    );
    """
...
def sincronizar_cabecalho_preferencia(conexao, config: ConfiguracaoPreferenciaReserva) -> dict[str, int]:
    ...
    INNER JOIN #ContratosPreferenciaReserva AS origem
        ON origem.IDFatoControleContratosMidia = destino.IDFatoControleContratosMidia
    WHERE
    ...
    SELECT
        origem.IDFatoControleContratosMidia,
        origem.IDEmpresa,
        origem.IDVendedor,
        origem.IDUsuario,
    ...
    (
        SELECT 1
        FROM {config.tabela_preferencia} AS destino
        WHERE destino.IDFatoControleContratosMidia = origem.IDFatoControleContratosMidia
    );
    """
...
def sincronizar_itens_preferencia(conexao, config: ConfiguracaoPreferenciaReserva) -> dict[str, int]:
    ...
    (
        SELECT
            origem_item.IDFatoControleContratosItensMidia,
            origem_item.IDFatoControleContratoMidia,
            origem_item.DataInicioPrevisto,
            origem_item.DataTerminoPrevisto,
            origem_item.IDPainelMidia,
            ...
            (
                PARTITION BY
                    origem_item.IDFatoControleContratoMidia,
                    origem_item.DataInicioPrevisto,
                    ISNULL(origem_item.IDPainelMidia, -1),
                    ISNULL(origem_item.IDDimFacesPaineis, -1)
                ORDER BY
                    origem_item.DataTerminoPrevisto DESC,
                    origem_item.IDFatoControleContratosItensMidia DESC
            ) AS OrdemItem
        FROM #ItensPreferenciaReserva AS origem_item
    )
    UPDATE destino_item
    ...
    INNER JOIN origem_unica AS origem_item
        ON origem_item.IDFatoControleContratoMidia = destino.IDFatoControleContratosMidia
    ...
    INNER JOIN {config.tabela_preferencia} AS destino
        ON destino.IDFatoControleContratosMidia = origem_item.IDFatoControleContratoMidia
    WHERE
        ...
        (
            ...
            WHERE
                ...
                AND ISNULL(destino_item.IDPainelMidia, -1) = ISNULL(origem_item.IDPainelMidia, -1)
...
@dag(
    ...
    catchup=False,
    tags=["Midia", "Contratos", "Preferencia", "Reserva", "SQL Server"],
    max_active_runs=1,
