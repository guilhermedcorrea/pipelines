...
def pasta_pdf_local_contrato_d4() -> Path:
    ...
    pasta = str(
        ...
        or "/home/midia/projetos/pipelines/FlaskApp/Contratos/Midia/Anexos/Contrato"
    ).strip()
    return Path(pasta)
...
def garantir_pdf_local_contrato_d4(conexao, contrato: dict[str, Any], nome_documento_d4: str | None = None) -> dict[str, Any]:
    """Garante exatamente o que a tela precisa: PDF físico + linha na FatoArquivosContratosMidia."""
...
def atualizar_pdf_local_assinado_contrato_d4(
    ...
    O nome do arquivo é preservado quando já existe linha em FatoArquivosContratosMidia, para
...
def garantir_pdfs_locais_contratos_d4(hook_sql: HookSqlServer) -> dict[str, Any]:
    ...
    contratos = hook_sql.executar_select(
        ...
        WHERE ISNULL(d.BitAtivo, 1) = 1
          AND d.IDFatoControleContratosMidia IS NOT NULL
          AND NULLIF(LTRIM(RTRIM(d.UUIDDocumentoD4)), '') IS NOT NULL
        ORDER BY
            CASE WHEN EXISTS (
                SELECT 1
                  FROM {TABELA_ARQUIVOS_CONTRATOS} a
                 WHERE a.IDFatoControleContratosMidia = d.IDFatoControleContratosMidia
...
TAGS_DAG = ["Midia", "Contratos", "D4Sign", "Webhook", "SQLServer"]
...
def buscar_contrato_por_uuid(conexao, uuid_documento: str) -> dict[str, Any] | None:
    ...
    Também resolvo o IDFatoKanbanCard pelo vínculo em FatoContratoCardMidia quando
...
def resolver_status_final_midia(status_atual: Any, status_novo: Any) -> int | None:
...
def propagar_status_contrato_midia(
    ...
    *,
    ...
    status_final = resolver_status_final_midia(status_atual, id_status_novo)
...
def processar_evento(conexao, evento: dict[str, Any]) -> dict[str, Any]:
    ...
    if propagacao_midia.get("propagou") and propagacao_midia.get("status_final") is not None:
        status_evento_historico["IDDimStatusContratos"] = propagacao_midia.get("status_final")
    ...
    return {
        ...
        "id_status_contrato": status_evento.get("IDDimStatusContratos"),
        "propagacao_midia": propagacao_midia,
