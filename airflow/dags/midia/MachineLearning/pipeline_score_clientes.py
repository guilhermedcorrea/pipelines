...
class Configuracao:
    ...
    caminho_query_sql: Path = Path(
        "/opt/airflow/queries/Midia/MachineLearning/algoritmo_score_cliente.sql"
    )

    pasta_saida_dados: Path = Path(
        "/opt/airflow/Dados/Midia/MachineLearning"
    )
    pasta_saida_metricas: Path = Path(
        "/opt/airflow/Artefatos/Midia/MachineLearning/Metricas"
    )
    pasta_saida_intermediarios: Path = Path(
        "/opt/airflow/Artefatos/Midia/MachineLearning/Intermediarios"
    )
    pasta_saida_historico_execucoes: Path = Path(
        "/opt/airflow/Artefatos/Midia/MachineLearning/HistoricoExecucoes"
    )

    caminho_dataset_bruto_parquet: Path = Path(
        "/opt/airflow/Artefatos/Midia/MachineLearning/Intermediarios/score_cliente_dataset_bruto.parquet"
    )
    caminho_dataset_preparado_parquet: Path = Path(
        "/opt/airflow/Artefatos/Midia/MachineLearning/Intermediarios/score_cliente_dataset_preparado.parquet"
    )
    caminho_metadados_preparacao_json: Path = Path(
        "/opt/airflow/Artefatos/Midia/MachineLearning/Intermediarios/score_cliente_metadados_preparacao.json"
    )
    caminho_resumo_treino_json: Path = Path(
        "/opt/airflow/Artefatos/Midia/MachineLearning/Intermediarios/score_cliente_resumo_treino.json"
    )
    caminho_saida_payload_metricas_json: Path = Path(
        "/opt/airflow/Artefatos/Midia/MachineLearning/Intermediarios/score_cliente_payload_metricas.json"
    )

    caminho_saida_score_historico_csv: Path = Path(
        "/opt/airflow/Dados/Midia/MachineLearning/score_cliente_historico.csv"
    )
    caminho_saida_score_atual_csv: Path = Path(
        "/opt/airflow/Dados/Midia/MachineLearning/score_cliente_atual.csv"
    )
    caminho_saida_metricas_json: Path = Path(
        "/opt/airflow/Artefatos/Midia/MachineLearning/Metricas/score_cliente_metricas.json"
    )
    caminho_saida_dashboard_spec_json: Path = Path(
        "/opt/airflow/Artefatos/Midia/MachineLearning/Metricas/score_cliente_dashboard_spec.json"
    )
    caminho_saida_importancias_csv: Path = Path(
        "/opt/airflow/Dados/Midia/MachineLearning/score_cliente_importancias.csv"
    )
    caminho_saida_resumo_faixas_csv: Path = Path(
        "/opt/airflow/Dados/Midia/MachineLearning/score_cliente_faixas.csv"
    )
    caminho_saida_tabela_final_csv: Path = Path(
        "/opt/airflow/Dados/Midia/MachineLearning/score_cliente_tabela_final.csv"
    )
    caminho_saida_walk_forward_folds_csv: Path = Path(
        "/opt/airflow/Dados/Midia/MachineLearning/score_cliente_walk_forward_folds.csv"
    )
    caminho_saida_walk_forward_predicoes_csv: Path = Path(
        "/opt/airflow/Dados/Midia/MachineLearning/score_cliente_walk_forward_predicoes.csv"
    )
...
@dag(
    ...
    catchup=False,
    tags=["Midia", "MachineLearning", "Comercial", "Empresas"],
    max_active_runs=1,
)
