...
(() => {
  ...
  function atualizarResumoDisponibilidadeDoBloco(bloco){
    ...
    if (!dataInicio && dataFim){
      atualizarMensagemReservaDoBloco(bloco, 'Preencha primeiro a Data de.', 'erro');
      return;
    }

    if (dataFim < dataInicio){
      atualizarMensagemReservaDoBloco(bloco, 'A Data até não pode ser menor que a Data de.', 'erro');
      return;
    }
...
function redesenharFasesLocalmente(idsFase, mapaQuantidades = null, manterScroll = true){
  ...
  function atualizarVisibilidadeEmpresasRelacionadasCard(){
    ...
    const mostrarWrapRelacionadas = !!config.mostrarWrap && (exibirRelacionadasNaFase4 || mostrarAgencia || mostrarClienteDireto || mostrarBureau);
...
function formatarNumeroParaInput(valor){
  ...
  function preencherSelectOpcoesSimples(selectEl, opcoes, placeholder){
    ...
    if (valorAtual && (Array.isArray(opcoes) ? opcoes : []).some((opcao) => safeStr(opcao?.valor ?? '').trim() === valorAtual)) {
      selectEl.value = valorAtual;
    }
  }
  ...
  function localizarPrecoPorFiltrosDoBloco(bloco, filtros = {}){
    ...
    if (idPreferido) {
      const encontrado = candidatos.find((preco) => idNum(preco?.IDDimTabelaPrecosEuromidia) === idPreferido);
  ...
  function ocultarCamposTabelaPrecoDoBloco(bloco){
    ...
    if (selectPeriodo) preencherSelectOpcoesSimples(selectPeriodo, [], '— Período —');
  }
  ...
  function sincronizarSeletoresTabelaPrecoDoBloco(bloco, opcoes = {}){
    ...
    let exibicoesDesejadas = safeStr(selectExibicoes.value || '').trim();
    let periodoDesejado = safeStr(selectPeriodo.value || '').trim();

    if (!exibicoesDesejadas && precoPreferido) {
      exibicoesDesejadas = chaveExibicoesDiaDoPreco(precoPreferido, usarInsercoesDigitais);
    }
    if (!periodoDesejado && precoPreferido) {
      periodoDesejado = safeStr(precoPreferido?.PeriodoExibicao || '').trim();
    }

    let opcoesExibicoes = listarOpcoesExibicoesDiaDoBloco(bloco, periodoDesejado);
    if (!opcoesExibicoes.length) {
      opcoesExibicoes = listarOpcoesExibicoesDiaDoBloco(bloco, '');
    }

    if (!opcoesExibicoes.some((opcao) => opcao.valor === exibicoesDesejadas)) {
      exibicoesDesejadas = opcoesExibicoes[0]?.valor || '';
    }

    let opcoesPeriodo = listarOpcoesPeriodoDoBloco(bloco, exibicoesDesejadas);
    ...
    if (!opcoesPeriodo.some((opcao) => opcao.valor === periodoDesejado)) {
      periodoDesejado = opcoesPeriodo[0]?.valor || '';
    }

    opcoesExibicoes = listarOpcoesExibicoesDiaDoBloco(bloco, periodoDesejado);
    if (!opcoesExibicoes.some((opcao) => opcao.valor === exibicoesDesejadas)) {
      exibicoesDesejadas = opcoesExibicoes[0]?.valor || '';
    }
    ...
    const precoSelecionado = localizarPrecoPorFiltrosDoBloco(bloco, {
      ...
      exibicoes_dia: exibicoesDesejadas,
    ...
    preencherSelectOpcoesSimples(selectPeriodo, opcoesPeriodo, '— Período —');

    if (precoSelecionado) {
      const exibicoesFinais = chaveExibicoesDiaDoPreco(precoSelecionado, usarInsercoesDigitais);
      const periodoFinal = safeStr(precoSelecionado?.PeriodoExibicao || '').trim();
      selectExibicoes.value = exibicoesFinais;
      selectPeriodo.value = periodoFinal;
      definirValorSelectOculto(
        selectPreco,
        String(idNum(precoSelecionado?.IDDimTabelaPrecosEuromidia || 0) || ''),
        `${safeStr(precoSelecionado?.PeriodoExibicao || '—')} | ${labelExibicoesDiaDoPreco(exibicoesFinais)} | ${formatarMoedaBR(precoSelecionado?.Valor)}`
      );
    } else {
      limparSelectPrecosNoElemento(selectPreco);
    }
    ...
    const bloco = el('div', { class:'kb-painel-item' }, [
      ...
      el('div', { class:'kb-painel-item-grid' }, [
        ...
        el('div', { class:'kb-campo', 'data-role':'wrap-exibicoes-dia', hidden:'hidden' }, [
          ...
          el('select', { class:'kb-select', 'data-role':'select-exibicoes-dia' }, [
            el('option', { value:'' }, ['— Inserções / dia —'])
          ])
        ]),
        el('div', { class:'kb-campo', 'data-role':'wrap-periodo-exibicao', hidden:'hidden' }, [
          el('div', { class:'kb-campo-label' }, ['Período']),
          el('select', { class:'kb-select', 'data-role':'select-periodo-exibicao' }, [
            el('option', { value:'' }, ['— Período —'])
          ])
        ])
      ]),
  ...
  function obterPrecoSelecionadoDoBloco(bloco){
    const idPreco = idNum(bloco.querySelector('[data-role="select-preco"]')?.value || 0);
...
function atualizarResumoComercial(bloco, opcoes = {}){
  ...
  const valorTabelaSelecionada = Number(precoSel?.Valor);
  const valorBaseTabela = Number.isFinite(valorTabelaSelecionada) ? valorTabelaSelecionada : null;
...
function renderizarComercialBloco(bloco, comercial, valoresSalvos = null){
  ...
  wrap.appendChild(el('div', { class:'kb-painel-item-grid-2' }, [
    ...
    el('div', { class:'kb-campo' }, [
      el('div', { class:'kb-campo-label' }, ['Preço de venda atual']),
  ...
  wrap.appendChild(el('div', { class:'kb-row' }, [
    el('div', { class:'kb-campo grow' }, [
      el('div', { class:'kb-campo-label' }, ['Data de']),
      el('input', {
        class:'kb-input',
  ...
  wrap.appendChild(el('div', { class:'kb-painel-item-grid-3' }, [
    ...
    el('div', { class:'kb-campo' }, [
      el('div', { class:'kb-campo-label' }, ['Valor final']),
...
async function moverCard(idCard, idFasePara, posicao) {
  ...
  try {
    ...
    if (idNum(cardAbertoId) === idC && modalCard?.style?.display === "block") {
      modalCard.dataset.idFaseAtual = String(idNum(cardFinal.IDDimKanbanFaseAtual || idDestino) || "");
      atualizarVisibilidadeFormularioSolicitacaoContrato();
        ...
        const artigo = el("article", {class:"kb-orcamento-item"}, [
          ...
          el("div", {class:"kb-orcamento-item-info"}, [
            ...
            el("div", {class:"kb-orcamento-dados-grid"}, [
              ...
              criarCampoOrcamento("Período de exibição", safeStr(item.periodo_exibicao || "Período não informado") + (item.exibicoes_dia ? ` • ${item.exibicoes_dia} exibições/dia` : ""))
            ])
          ])
        ]);
  ...
  function validarReservasPainelFacesFormulario(){
    for (const bloco of (painelFaceLista?.querySelectorAll('.kb-painel-item') || [])){
      ...
      if ((dataInicio && !dataFim) || (!dataInicio && dataFim)){
        return { ok: false, msg: `${titulo}: preencha Data de e Data até.` };
      }

      if (dataInicio && dataFim && dataFim < dataInicio){
        return { ok: false, msg: `${titulo}: a Data até não pode ser menor que a Data de.` };
      }
