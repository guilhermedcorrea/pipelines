...
from ..models.euro_models import (Produto,Caracteristica,CategoriasProdutos,Saldo,Ativo
                                 ...
                                 ,EstoqueShempo,Departamento,GruposCompativeis,Pmv,EstoqueManutencaoInterna,EstoqueContainer,
...
euro = Blueprint('euro', __name__, url_prefix='/admin/shempo/v1')
...
def _shempo_execute(statement, params=None):
    """Executa SQL textual no bind Shempo dentro da sessão atual."""
    return db.session.execute(
        statement,
        params,
        bind_arguments={"bind": _shempo_engine()},
    )
...
def _id_usuario_compatibilidade_euro():
    """Mantém o ID usado nas rotinas antigas do Shempo sem autenticar por ele."""
...
def lista_produtos():
    ...
    consultas = [
        ...
        (EstoqueShempo,'EstoqueShempo', EstoqueShempo.Saldo),
    ]
...
def ver_movimentacao(item_id):
    ...
    SELECT DISTINCT
        ...
        M.Quantidade,
        CASE
            WHEN M.TipoEstoqueOrigem = 'EstoqueEuroMidia' THEN
                 COALESCE(PEOrigem.Logradouro, SHOrigem.Logradouro)
                 + ' - ' + CAST(M.CodPontoOrigem AS VARCHAR(20))
            WHEN M.TipoEstoqueOrigem = 'EstoqueShempo' THEN
                 COALESCE(PEOrigem.Logradouro, SHOrigem.Logradouro)
                 + ' - ' + CAST(M.CodPontoOrigem AS VARCHAR(20))
        ...
        M.CodPontoOrigem,
        CASE
            WHEN M.TipoEstoqueDestino = 'EstoqueEuroMidia' THEN 'EstoqueEuroMidia'
            WHEN M.TipoEstoqueDestino = 'EstoqueEuro' THEN 'EstoqueEuro'
            WHEN M.TipoEstoqueDestino = 'EstoqueShempo' THEN 'EstoqueShempo'
    ...
    LEFT JOIN EstoqueShempo ESO
           ON ESO.ShempoId = M.IDProprietarioOrigem
          AND ESO.CodPonto = M.CodPontoOrigem
    LEFT JOIN Shempo SHOrigem
           ON SHOrigem.ShempoId = ESO.ShempoId
    ...
    LEFT JOIN EstoqueShempo ESD
           ON ESD.ShempoId = M.IDProprietarioDestino
          AND ESD.CodPonto = M.CodPontoDestino
    LEFT JOIN Shempo SHDest
           ON SHDest.ShempoId = ESD.ShempoId
          AND SHDest.CodPonto = ESD.CodPonto
    WHERE M.IDItem = :item_id
    ORDER BY M.IDMovimentacao DESC
    """

    result = _shempo_execute(text(query_str), {"item_id": item_id})
...
def listar_familias():
    ...
    return render_template('shempo/familias.html', familias=familias)
...
def produto_compativeis(produto_id):
    ...
    return render_template(
        'shempo/produto_compativeis.html',
        prod = prod,
        produtos = produtos,
        page = page,
        total_pages  = total_pages,
...
@login_required
def familia_4600():
    return render_template('shempo/familia_4600.html')
...
def pecas_componentes():
    ...
    return render_template('shempo/pecas_componentes.html', items=items, page=page, total_pages=total_pages, search=search)
...
def estoque_manut_externa_detail(item_id):
    ...
    """)
    details = _shempo_execute(detail_query, {"item_id": item_id}).fetchall()

   
    total_externa = sum(row.Saldo for row in details) if details else 0

    return render_template(
        'shempo/estoque_manut_externa_detail.html',
        product=product,
...
def estoque_manut_interna_detail(item_id):
    ...
    return render_template(
        'shempo/estoque_manut_interna_detail.html',
        product=product,
...
def estoque_container_detail(item_id):
    ...
    return render_template(
        'shempo/estoque_container_detail.html',
        product=product,
...
def ver_estoques(item_id):
    ...
    estoque_euromidia= db.session.query(EstoqueEuro).filter_by(IDItem=item_id).all()
    estoque_euromatriz= db.session.query(EstoqueEuroMatriz).filter_by(IDItem=item_id).all()
    estoque_shempo= db.session.query(EstoqueShempo).filter_by(IDItem=item_id).all()
    ...
    total_euromidia  = sum(e.Saldo for e in estoque_euromidia) if estoque_euromidia else 0
    total_euromatriz = sum(e.Quantidade for e in estoque_euromatriz) if estoque_euromatriz else 0
    total_shempo = sum(e.Saldo for e in estoque_shempo) if estoque_shempo else 0
    ...
    return render_template(
        ...
        total_euromidia=total_euromidia,
        estoque_euromatriz=estoque_euromatriz,
        total_euromatriz=total_euromatriz,
        estoque_shempo=estoque_shempo,
        total_shempo=total_shempo,
...
def movimentar_estoque(item_id):
    ...
    shempo_reg = db.session.query(EstoqueShempo).filter_by(IDItem=item_id).all()
    ...
    for e in euro_reg: e.NomeEstoque = 'EstoqueEuroMidia'
    for e in euromat_reg:e.NomeEstoque = 'EstoqueEuroMatriz'
    for e in shempo_reg: e.NomeEstoque = 'EstoqueShempo'
    ...
    existentes = (
        matriz + euro_reg + euromat_reg + shempo_reg +
    ...
    def to_dict(e):
        nome = e.NomeEstoque
        prop = (
            e.EuroID if nome in ('EstoqueEuroMidia', 'EstoqueEuroMatriz') else
            e.ShempoId if nome == 'EstoqueShempo' else 0
        )
        return {
    ...
    for r in registros:
        ...
        if r['NomeEstoque'] != 'EstoqueEuroMidia':
            destino_groups.setdefault(r['NomeEstoque'], []).append(r)

    
    pontos_euro = _shempo_execute(text(
    ...
    destino_groups['EstoqueEuroMidia']  = list({r['CodPonto']: r for r in euro_opts}.values())
    destino_groups['EstoqueEuroMatriz'] = list({r['CodPonto']: r for r in euro_opts}.values())

    for nome in (
        'EstoqueMatriz','EstoqueShempo',
    ...
    if item.ControlaLote:
        ...
        for row in _shempo_execute(text(sql), {'item_id': item_id}):
    ...
    if request.method == 'POST':
        ...
        def resolve(nome: str) -> str:
            return {
                'estoqueeuromidia':'EstoqueEuroMidia',
                'estoqueeuro':'EstoqueEuroMidia',
                'estoqueeuromatriz': 'EstoqueEuroMatriz',
                'estoqueshempo': 'EstoqueShempo',
        ...
        try:
            with db.session.begin_nested():
                ...
                if destino:
                    ...
                    if tipo_d in ('EstoqueEuroMidia', 'EstoqueEuroMatriz'):
                        kwargs.update(EuroID=prop_id, CodPonto=cod)
                    elif tipo_d == 'EstoqueShempo':
                        kwargs.update(ShempoId=prop_id)
            ...
            with _shempo_engine().begin() as conn:
                conn.execute(
                    text("""
    ...
    return render_template(
        'shempo/movimentar_estoque_transfer.html',
        item = item,
...
def estoque_euromidia_detail(item_id):
    ...
    """)
    total_result = _shempo_execute(total_query, {"item_id": item_id}).fetchone()
    total_euromidia = total_result.TotalSaldo if total_result and total_result.TotalSaldo is not None else 0
    ...
    return render_template(
        'shempo/estoque_euromidia_detail.html',
        product=product,
        total_euromidia=total_euromidia,
        details=details
    )
...
def estoque_shempo_detail(item_id):
    ...
    detail_query = text("""
       SELECT 
           ...
           B.Logradouro,
           B.Faces
       FROM EstoqueShempo AS A
       LEFT JOIN Shempo AS B 
    ...
    total_shempo = sum(row.Saldo for row in details) if details else 0

    return render_template(
        'shempo/estoque_shempo_detail.html',
        product=product,
        total_shempo=total_shempo,
        details=details
    )
...
def vincular_componentes():
    if request.method == 'POST':
        ...
        return render_template('shempo/vincular_componentes.html', pmvs=pmvs)
...
def componentes_vinculados():
    ...
    return render_template(
        "shempo/componentes_vinculados.html",
...
def adicionar_saldos():
        ...
        for idx, line in enumerate(lines_parsed, start=1):
            try:
                ...
                if nome_estoque in ('EstoqueEuro', 'EstoqueEuroMatriz'):
                    ...
                    try:
                        shempo_id, cod_ponto = map(int, cod_ponto_val.split('|'))
                    except Exception:
                        flash(f"[Linha {idx}] Formato inválido de ponto para EstoqueShempo: '{cod_ponto_val}'.", "error")
                        continue
                    estoque_reg = db.session.query(Model).filter_by(
                        IDItem=id_item, ShempoId=shempo_id, CodPonto=cod_ponto
                    ).first()
                ...
                if operacao == 'entrada' and not estoque_reg:
                    ...
                    if nome_estoque in ('EstoqueEuro', 'EstoqueEuroMatriz'):
                        ...
                        kwargs.update(ShempoId=shempo_id,
                                      CodPonto=cod_ponto,
                                      Saldo=quantidade)
    ...
    pontos_euro_raw = _shempo_execute(text(
        "SELECT EuroID, NomeEstoque, CodPonto, Logradouro FROM PontosEuro"
    )).fetchall()
    pontos_euro = [
    ...
    pontos_shempo_raw = _shempo_execute(text(
        "SELECT ShempoId AS ID, CodPonto, Logradouro FROM Shempo"
    )).fetchall()
    pontos_shempo = [
        dict(ID=r.ID,
             CodPonto=r.CodPonto,
             Logradouro=r.Logradouro)
        for r in pontos_shempo_raw
    ]

    return render_template(
        'shempo/adicionar_saldos.html',
        pontos_euro=pontos_euro,
        pontos_shempo=pontos_shempo
    )
...
def movimentacao_produto():
    ...
    SELECT
      ...
      LEFT JOIN EstoqueShempo ESO
        ON ESO.ShempoId  = M.IDProprietarioOrigem
       AND ESO.CodPonto  = M.CodPontoOrigem
      LEFT JOIN Shempo SHO
        ON SHO.ShempoId   = ESO.ShempoId
      ...
      LEFT JOIN EstoqueShempo ESD
        ON ESD.ShempoId   = M.IDProprietarioDestino
       AND ESD.CodPonto  = M.CodPontoDestino
      LEFT JOIN Shempo SHD
        ON SHD.ShempoId   = ESD.ShempoId
    ...
    rows = _shempo_execute(text(main_sql), params).fetchall()
    ...
    if item_ids:
        ...
        for lot in _shempo_execute(lotes_sql, {"item_ids": list(item_ids)}):
    ...
    return render_template(
        "shempo/movimentacao_produto.html",
...
def disponibilidade_produtos():
    ...
    if item_id is not None:
        ...
        FROM [Shempo].[dbo].[EstoqueEuro] AS a
        INNER JOIN [Shempo].[dbo].[Produto] AS b 
            ON b.IDItem = a.IDItem
        WHERE a.IDItem = :item_id 
          AND a.Saldo > 0
        ...
        FROM [Shempo].[dbo].[EstoqueMatriz] AS a
        INNER JOIN [Shempo].[dbo].[Produto] AS b 
            ON b.IDItem = a.IDItem
        WHERE a.IDItem = :item_id 
          AND a.Saldo > 0
        ...
        FROM [Shempo].[dbo].[EstoqueContainer] AS a
        INNER JOIN [Shempo].[dbo].[Produto] AS b 
            ON b.IDItem = a.IDItem
        WHERE a.IDItem = :item_id 
          AND a.Saldo > 0
        ...
        FROM [Shempo].[dbo].[EstoqueManutencaoExterna] AS a
        INNER JOIN [Shempo].[dbo].[Produto] AS b 
            ON b.IDItem = a.IDItem
        WHERE a.IDItem = :item_id 
          AND a.Saldo > 0
        ...
        FROM [Shempo].[dbo].[EstoqueManutencaoInterna] AS a
        INNER JOIN [Shempo].[dbo].[Produto] AS b 
            ON b.IDItem = a.IDItem
        WHERE a.IDItem = :item_id 
          AND a.Saldo > 0
    ...
    return render_template("shempo/disponibilidade_produtos.html",
...
def saldo_por_estoque():
    ...
    if item_id:
        ...
        if estoque:
            ...
            if est_lower == 'estoqueeuro':
                ...
                results = records
            elif est_lower == 'estoqueshempo':
                records = db.session.query(EstoqueShempo).filter_by(IDItem=item_id).all()
                saldo = sum(r.Saldo for r in records) if records else 0
                results = records
    ...
    pontos_euro = _shempo_execute(text("SELECT EuroID, NomeEstoque, CodPonto, Logradouro FROM PontosEuro")).fetchall()
    pontos_shempo = _shempo_execute(text("SELECT ShempoId AS ID, CodPonto, Logradouro FROM Shempo")).fetchall()
    
    pontos_euro = [dict(row._mapping) for row in pontos_euro]
    pontos_shempo = [dict(row._mapping) for row in pontos_shempo]

    return render_template("shempo/saldo_por_estoque.html",
                           item_id=item_id,
                           estoque=estoque,
                           saldo=saldo,
                           produto=produto,
...
def estoquematriz():
    ...
    if store_type == 'Todos':
        ...
        rows.extend(fetch_rows(EstoqueShempo, EstoqueShempo.Saldo, EstoqueShempo.EstoqueMinimo))
    else:
        mapping = {
            ...
            'EstoqueShempo': (EstoqueShempo, EstoqueShempo.Saldo, EstoqueShempo.EstoqueMinimo),
        }
...
def ver_mais_curva_produto(item_id: int):
    ...
    return render_template(
        "shempo/ver_mais_curva_produto.html",
        item_id = item_id,
        nome_prod = nome_prod,
        ref_ext = ref_ext,
        period = period,
...
def add_to_cart(item_id):
    ...
    saldo_shempo = db.session.query(func.sum(EstoqueShempo.Saldo)) \
        .filter(EstoqueShempo.IDItem == item_id).scalar() or 0
...
def checkout():
    ...
    generic += db.session.query(EstoqueShempo).filter(
        EstoqueShempo.IDItem.in_(item_ids),
        EstoqueShempo.IDTipoEstoque.in_(allowed)
    ).all()
...
def formulario_retirada(pedido_id):
    ...
    return render_template(
        'shempo/formulario_retirada.html',
        pedido=pedido,
        funcionario=funcionario,  
