SET NOCOUNT ON;
    ...
    CREATE TABLE #faces (
        CodFace VARCHAR(50) NOT NULL
    );

    IF NULLIF(LTRIM(RTRIM(@CodFace)), '') IS NOT NULL
    BEGIN
        INSERT INTO #faces (CodFace)
        VALUES (CAST(LTRIM(RTRIM(@CodFace)) AS VARCHAR(50)));
    END
    ELSE
    BEGIN
        IF NULLIF(LTRIM(RTRIM(COALESCE(@FacesCsv,''))), '') IS NOT NULL
        BEGIN
            INSERT INTO #faces (CodFace)
        ...
        IF NOT EXISTS (SELECT 1 FROM #faces) AND NULLIF(LTRIM(RTRIM(@FacePadrao)), '') IS NOT NULL
        BEGIN
            INSERT INTO #faces (CodFace)
            VALUES (CAST(LTRIM(RTRIM(@FacePadrao)) AS VARCHAR(50)));
        END
    END
    ...
    IF NOT EXISTS (SELECT 1 FROM #faces_todas) AND NULLIF(LTRIM(RTRIM(@FacePadrao)), '') IS NOT NULL
    BEGIN
        INSERT INTO #faces_todas (CodFace)
        VALUES (CAST(LTRIM(RTRIM(@FacePadrao)) AS VARCHAR(50)));
    END
    ...
    AND (
            @CodFace IS NULL
            OR LTRIM(RTRIM(COALESCE(c.CodFace,''))) = @CodFace
    ...
    AND (
            @CodFace IS NULL
            OR LTRIM(RTRIM(COALESCE(r.CodFace,''))) = @CodFace
    ...
    INTO #pico
    FROM (
        SELECT Dia, SUM(OcupadoNoDia) AS OcupadoNoDiaPainel
        FROM #ocupacao_capada
        GROUP BY Dia
    ) p;
    ...
    AND TRY_CONVERT(INT, c.CodPonto) = @CodPonto
    AND c.DataInicioPrevisto IS NOT NULL
    AND c.DataInicioPrevisto <= @DtFim
    AND COALESCE(
            c.DataCancelamento,
            c.DataTerminoPrevisto,
            CONVERT(date, '9999-12-31')
    ...
    LEFT JOIN #custo_mensal cm
        ON cm.Ano = mp.Ano AND cm.Mes = mp.Mes
    LEFT JOIN #por_mes pm
        ON pm.Ano = mp.Ano AND pm.Mes = mp.Mes
    LEFT JOIN #ocup_total_mes_capada otm
        ON otm.Ano = mp.Ano AND otm.Mes = mp.Mes;
    ...
    cdi_fatores AS (
        SELECT (1.0 + (CdiDia / 100.0)) AS FatorDia
        FROM cdi_dias
    ),
    cdi_agregado AS (
        SELECT CAST(EXP(SUM(LOG(NULLIF(FatorDia, 0.0)))) AS decimal(18,10)) AS CdiFatorPeriodo
        FROM cdi_fatores
        WHERE FatorDia > 0
    )
    SELECT
    ...
    SELECT
        ...
        CAST(CASE WHEN @EhDigital = 1 THEN (t.QtdFaces * @KDigital) ELSE t.QtdFaces END AS INT) AS SlotsTotalDia,
        CAST(ISNULL(t.OcupadoSlotDiasTotal, 0.0) AS decimal(18,2)) AS OcupadoSlotDiasTotal,
        CAST(CASE WHEN @EhDigital = 1 THEN (t.QtdFaces * @KDigital * t.TotalDias) ELSE (t.QtdFaces * t.TotalDias) END AS decimal(18,2)) AS CapacidadeSlotDiasTotal,
        CAST(
            CASE
                WHEN (CASE WHEN @EhDigital = 1 THEN (t.QtdFaces * @KDigital * t.TotalDias) ELSE (t.QtdFaces * t.TotalDias) END) <= 0 THEN NULL
                ELSE (ISNULL(t.OcupadoSlotDiasTotal, 0) * 100.0) /
                    (CASE WHEN @EhDigital = 1 THEN (t.QtdFaces * @KDigital * t.TotalDias) ELSE (t.QtdFaces * t.TotalDias) END)
            END
...
sp500_ini AS (
    SELECT TOP (1) DataRef AS DataInicioEfetiva, UltimoBRL AS UltimoBRL_Inicio
    FROM sp500_base
    ORDER BY DataRef ASC
),
sp500_fim AS (
    SELECT TOP (1) DataRef AS DataFimEfetiva, UltimoBRL AS UltimoBRL_Fim
    FROM sp500_base
