...
def gerar_relatorio_ocupacao_clientes_excel_async(self, ano: int) -> dict:
    """
    Eu gero o Excel "Ocupação Clientes" em segundo plano.

    Esta task é adicional e não substitui a task da grade anual.
    """
    try:
        self.update_state(
            state="PROGRESS",
        ...
        self.update_state(
            state="PROGRESS",
            meta={
                "status": f"Consultando painéis, ocupações e preferências de renovação de {ano_int}...",
                "progresso": 35,
            },
        )

        bio, nome_download = _gerar_excel_ocupacao_clientes_bytes(ano_int)
        ...
        nome_seguro = f"{task_id}_ocupacao_clientes_{ano_int}.xlsx"
        caminho = pasta / nome_seguro
